...
removals_binance = ['DUSK', 'WBTC', 'BUSD', 'USDC', 'EUR', 'GBP', 'JPY', 'AUD', 'TRYB', 'XAUT', 'DOWN', 'UP', 'BULL', 'BEAR', 'LONG', 'SHORT', 'MOVE', ':USD', 'HALF', 'HEDGE', 'TUSD', 'USDS', 'TRY', 'RUB', 'ZAR', 'IDRT', 'UAH', 'BIDR', 'BKRW', 'DAI', 'BKRW', 'TRB', 'NGN', 'BRL', 'BVND', 'GYEN']
...
#         df_ftx = pd.DataFrame(exchange_ftx.fetch_ohlcv(crypto1, '1d', limit=1850), columns=['date', 'open', 'high', 'low', 'close', 'volume'])
...
# removals_coinbase = ['DUSK', 'WBTC', 'BUSD', 'WLUNA', 'USDC', 'USDT', 'EUR', 'GBP', 'INDEX' 'JPY', 'AUD', 'TRYB', 'XAUT', 'DOWN', 'UP', 'BULL', 'BEAR', 'LONG', 'SHORT', 'MOVE', ':USD', 'HALF', 'HEDGE', 'TUSD', 'USDS', 'TRY', 'RUB', 'ZAR', 'IDRT', 'UAH', 'BIDR', 'BKRW', 'DAI', 'BKRW', 'TRB', 'NGN', 'BRL', 'BVND', 'GYEN']
...
#         df_coinbase = pd.DataFrame(exchange_coinbase.fetch_ohlcv(crypto2, '1d', limit=1850), columns=['date', 'open', 'high', 'low', 'close', 'volume'])
...
# removals_okex = ['DUSK', 'JOY', 'EXO','BMN','ALBT','MDOGE','WAMPL','INDEX', 'USDT/' 'WinToken', 'Unitrade', 'DefiBox', 'EUR', 'CUSD', 'GBP', 'JPY', 'AUD', 'TRYB', 'XAUT', 'DOWN', 'UP', 'BULL', 'BEAR', 'LONG', 'SHORT', 'MOVE', ':USD', 'HALF', 'HEDGE', 'TUSD', 'USDS', 'TRY', 'RUB', 'ZAR', 'IDRT', 'UAH', 'BIDR', 'BKRW', 'DAI', 'BKRW', 'TRB', 'NGN', 'BRL', 'BVND', 'GYEN']
...
#         df_okex = pd.DataFrame(exchange_okex.fetch_ohlcv(crypto3, '1d', limit=1850), columns=['date', 'open', 'high', 'low', 'close', 'volume'])
...
# removals_cryptocom = ['DUSK', 'USDC', 'JOY', 'EXO','BMN','ALBT','MDOGE','WAMPL','INDEX', 'CUSD','BUSD', '2S', '3S', '4S', '5S', '2L', '3L', '4L', '5L', 'WinToken', 'Unitrade', 'DefiBox', 'EUR', 'GBP', 'JPY', 'AUD', 'TRYB', 'XAUT', 'DOWN', 'UP', 'BULL', 'BEAR', 'LONG', 'SHORT', 'MOVE', ':USD', 'HALF', 'HEDGE', 'TUSD', 'USDS', 'TRY', 'RUB', 'ZAR', 'IDRT', 'UAH', 'BIDR', 'BKRW', 'DAI', 'BKRW', 'TRB', 'NGN', 'BRL', 'BVND', 'GYEN']
...
#         df_cryptocom = pd.DataFrame(exchange_cryptocom.fetch_ohlcv(crypto4, '1d', limit=1850), columns=['date', 'open', 'high', 'low', 'close', 'volume'])
...
# removals_bitfinex = ['DUSK', 'JOY', 'EXO','BMN','ALBT','MDOGE','WAMPL','INDEX', 'USDC', 'CUSD', 'UST', 'USDT', 'B21X', '2.S', '2X', 'BUSD', 'MIM', 'XCHF', '2S', '3S', '4S', '5S', '2L', '3L', '4L', '5L', 'WinToken', 'Unitrade', 'DefiBox', 'EUR', 'GBP', 'JPY', 'AUD', 'TRYB', 'XAUT', 'DOWN', 'UP', 'BULL', 'BEAR', 'LONG', 'SHORT', 'MOVE', ':USD', 'HALF', 'HEDGE', 'TUSD', 'USDS', 'TRY', 'RUB', 'ZAR', 'IDRT', 'UAH', 'BIDR', 'BKRW', 'DAI', 'BKRW', 'TRB', 'NGN', 'BRL', 'BVND', 'GYEN']
...
#         df_bitfinex = pd.DataFrame(exchange_bitfinex.fetch_ohlcv(crypto5, '1d', limit=1850), columns=['date', 'open', 'high', 'low', 'close', 'volume'])
...
#         asset_dict_bitfinex[crypto5] = df_bitfinex
    ...
    ## HUOBI ##     ## USDT ##  ## CCXT API CURRENTLY NOT WORKING? ##
...
# removals_huobi = ['DUSK', 'JOY', 'EXO','BMN','ALBT','MDOGE','WAMPL','INDEX','USDC', 'CUSD', 'NHBTC', 'HOLO', 'HitChain', 'HUSD', 'BUSD', '1S', 'CTXC2X', 'Hydro Protocol', '2S', '3S', '4S', '5S', '2L', '3L', '4L', '5L', 'WinToken', 'Unitrade', 'DefiBox', 'EUR', 'GBP', 'JPY', 'AUD', 'TRYB', 'XAUT', 'DOWN', 'UP', 'BULL', 'BEAR', 'LONG', 'SHORT', 'MOVE', ':USD', 'HALF', 'HEDGE', 'TUSD', 'USDS', 'TRY', 'RUB', 'ZAR', 'IDRT', 'UAH', 'BIDR', 'BKRW', 'DAI', 'BKRW', 'TRB', 'NGN', 'BRL', 'BVND', 'GYEN']
...
#         df_huobi = pd.DataFrame(exchange_huobi.fetch_ohlcv(crypto6, '1d', limit=1850), columns=['date', 'open', 'high', 'low', 'close', 'volume'])
...
# exchange_gateio = ccxt.gateio({'options': {'defaultType': 'spot', 'adjustForTimeDifference': True}})
# ticker_gateio = exchange_gateio.fetch_tickers()
# asset_dict_gateio = dict()
# removals_gateio = ['DUSK', 'USDC', 'CUSD', 'SUSD', 'BONDED', 'Axis DeFi', 'CNYX', 'EOS', 'Bitcoin File', 'Goose Finance', 'Rai Reflex Index', 'Super Bitcoin', 'Trinity Network Credit', 'Game.com', 'BUSD', '2S', '3S', '4S', '5S', '2L', '3L', '4L', '5L', 'WinToken', 'Unitrade', 'DefiBox', 'EUR', 'GBP', 'JPY', 'AUD', 'TRYB', 'XAUT', 'DOWN', 'UP', 'BULL', 'BEAR', 'LONG', 'SHORT', 'MOVE', ':USD', 'HALF', 'HEDGE', 'TUSD', 'USDS', 'TRY', 'RUB', 'ZAR', 'IDRT', 'UAH', 'BIDR', 'BKRW', 'DAI', 'BKRW', 'TRB', 'NGN', 'BRL', 'BVND', 'GYEN']

# assets_gateio = pd.DataFrame(list(exchange_gateio.fetch_tickers()))
# assets_gateio = assets_gateio[assets_gateio[0].str.contains('USD')==True]
# assets_gateio = assets_gateio[0].values.tolist()
# updated_gateio_assets_tmp = [x for x in assets_gateio if not any(y in x for y in removals_gateio)]
# updated_gateio_assets = [coin.split('/USD')[0] + "/USD" for coin in updated_gateio_assets_tmp]
# total_gateio = list(set(updated_gateio_assets) - set(updated_binance_assets) - set(updated_ftx_assets) - set(updated_coinbase_assets) - set(updated_okex_assets) - set(updated_cryptocom_assets) -set(updated_bitfinex_assets) -set(updated_huobi_assets))
# total_gateio_USDT = [coin.split('/USD')[0] + "/USDT" for coin in total_gateio]

# for crypto7 in total_gateio_USDT:
#     try:
#         df_gateio = pd.DataFrame(exchange_gateio.fetch_ohlcv(crypto7, '1d', limit=1850), columns=['date', 'open', 'high', 'low', 'close', 'volume'])
#         df_gateio['date'] = pd.to_datetime(df_gateio['date'], unit='ms')
#         df_gateio = df_gateio.iloc[::-1]
#         df_gateio = df_gateio[df_gateio['date'] != today_utc]
#         with pd.ExcelWriter(f"{os.path.dirname(__file__)}/master-py-api/{crypto7.replace('/','').replace('USDT', 'USD')}.xlsx", datetime_format='DD-MM-YYYY') as writer: # engine={'xlsxwriter': True}
#             df_gateio.to_excel(writer, index=False)
#         asset_dict_gateio[crypto7] = df_gateio
#     except:
#         pass
...
# excel_binance_ftx_okex_coinbase = list((excel_tickers_binance) + (excel_tickers_ftx) + (excel_tickers_okex) + (excel_tickers_coinbase))
# excel_cryptocom_bitfinex_huobi_coinbase = list((excel_tickers_coinbase) + (excel_tickers_cryptocom) + (excel_tickers_bitfinex) + (excel_tickers_huobi))
# excel_tickers_huobi_cryptocom = list((excel_tickers_huobi)+(excel_tickers_cryptocom))
...
# print(f"coinbase {excel_tickers_coinbase}")
# print(f"cryptocom {excel_tickers_cryptocom}")
# print(f"bitfinex {excel_tickers_bitfinex}")
# print(f"huobi {excel_tickers_huobi}")
# print(f"total_huobi {total_huobi}")
...
# for crypto in excel_cryptocom_bitfinex_huobi_coinbase:
#     try:
#         xl = win32com.client.Dispatch("Excel.Application")
#         code = 'Sub CreateDaily() \n \
...
#                 End Sub'
#         path = (f"{os.path.dirname(__file__)}\\MCCHARTS\\{crypto}.xlsm")
#         wb = xl.Workbooks.Open(path)
#         wb.VBProject.VBComponents(1).CodeModule.AddFromString(code)
#         xl.Visible = False
#         #xl.Application.Run(f'{crypto}.xlsm!Module1.Delete14')
#         wb.Close(True)
#         xl.Quit()
#     except:
#         pass
# for crypto in excel_cryptocom_bitfinex_huobi:
#     try:
#         xl = win32com.client.Dispatch("Excel.Application")
#         code1 = 'Sub Delete14() \n \
#                     Application.ScreenUpdating = False \n \
...
# total_all = total_binance + total_ftx + total_coinbase + total_okex + total_cryptocom + total_bitfinex + total_huobi + total_gateio
...
# print(f"total number of all TICKERS used: {len(total_tickers)}")
